...
class DummyEpochBasedRunner(EpochBasedRunner):
    ...
    def run(self, data_loaders, workflow, **kwargs):
        ...
        self.logger.info(
            'workflow: %s, max: %d epochs', workflow, self._max_epochs
        )
...
class DummyIterBasedRunner(IterBasedRunner):
    ...
    def run(self, data_loaders, workflow, **kwargs):
        ...
        self.logger.info(
            'workflow: %s, max: %d iters', workflow, self._max_iters
        )
        self.call_hook('before_run')
...
def plot_curve(lr_list, args, iters_per_epoch, by_epoch=True):
    ...
    if by_epoch:
        ...
        sec_ax = ax.secondary_xaxis(
            'bottom',
            functions=(
                lambda x: x / iters_per_epoch, lambda y: y * iters_per_epoch
            )
        )
    ...
    if args.title is None:
        plt.title(f'{os.path.basename(args.config)} Learning Rate Curve')
    else:
