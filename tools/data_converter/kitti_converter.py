...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    Args:
        ...
        mono3d (bool, optional): Whether to export mono3d annotation.
            Default: True.
    """
    print('excito export_2d_annotation!')
...
def get_2d_boxes(info, occluded, mono3d=True):
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        # mark valid corners (depth > 0) as 1
        # (label but not visible, similar as COCO)
        ...
        corner_coords = view_points(corners_3d, camera_intrinsic,
                                    True).T[:, :2].tolist()
        all_corner_coords = view_points(all_corners_3d, camera_intrinsic,
                                        True).T[:, :2]
        all_corner_coords = np.hstack([all_corner_coords, valid_corners_mask])

        # Keep only corners that fall within the image.
        sample_shape = (1248, 384)
        final_coords = post_process_coords(corner_coords, sample_shape)
        ...
        min_x, min_y, max_x, max_y = final_coords  # use box2d_proj
        # min_x, min_y, max_x, max_y = ann_rec['bbox']  # use box2d_anno
        ...
        if mono3d and (repro_rec is not None):
            box_cam3d_p2 = np.concatenate([loc_3d, dim, rot], axis=1)
            repro_rec['bbox_cam3d'] = box_cam3d_p2.astype(
                np.float32).squeeze().tolist()
            box_cam3d_p0 = np.concatenate([loc, dim, rot], axis=1)
            repro_rec['bbox_cam3d_cam0'] = box_cam3d_p0.astype(
                np.float32).squeeze().tolist()
            ...
            for kpt_idx in range(len(projected_pts)):
                ...
                is_kpt_in_image = (0 <= kptx <= sample_shape[0]) and \
                                  (0 <= kpty <= sample_shape[1])
                if is_kpt_in_image:
