...
class IDAUp(nn.Module):
    ...
    def forward(self, layers):
        assert len(self.in_channels_list) == len(layers), \
            f'{len(self.in_channels_list)}-{len(layers)}'

        for i in range(1, len(layers)):
            up = getattr(self, 'up_' + str(i))
            proj = getattr(self, 'proj_' + str(i))
            node = getattr(self, 'node_' + str(i))
            layers[i] = up(proj(layers[i]))
...
class MonoConDLAUp(nn.Module):
    ...
    def __init__(
        ...
        super(MonoConDLAUp, self).__init__()
