...
class MonoConGaussianFocalLoss(nn.Module):
    ...
    def __init__(self, loss_weight=1.0, gamma=2.0, beta=4.0, alpha=-1):
        super(MonoConGaussianFocalLoss, self).__init__()
    ...
    def forward(self, source, target, **kwargs):
        """
        Args:
            source (torch.Tensor): Bbox predictions with shape [..., 3].
        ...
        pos_loss = torch.log(source + eps) * \
            torch.pow(1 - source, self.gamma) * pos_inds
        neg_loss = torch.log(1 - source + eps) * \
            torch.pow(source, self.gamma) * neg_weights * neg_inds
