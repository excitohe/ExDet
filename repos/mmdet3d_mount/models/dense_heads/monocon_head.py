...
from mmdet.models.utils.gaussian_target import (
    gaussian_radius, gen_gaussian_target, get_local_maximum,
    get_topk_from_heatmap, transpose_and_gather_feat
)
from repos.mmdet3d_mount.ops.attn_norm import AttnBatchNorm2d
...
class MonoConHead(nn.Module):
    ...
    def __init__(
        ...
        self.vis_level_regs = vis_level_regs

        self.use_attn_norm = use_attn_norm
        self.num_attn_norm = num_attn_norm
        self.norm = AttnBatchNorm2d if use_attn_norm else nn.BatchNorm2d
        ...
        self.center2kpt_offset_head = self._build_head(
            in_channels, feat_channels, self.num_kpts * 2
        )
        self.kpt_heatmap_head = self._build_head(
            in_channels, feat_channels, self.num_kpts
        )
        self.kpt_heatmap_offset_head = self._build_head(
            in_channels, feat_channels, 2
        )
        ...
        self.loss_ctr2d_hmp = build_loss(loss_ctr2d_hmp)
        self.loss_dim2d = build_loss(loss_dim2d)
        self.loss_ctr2d_res = build_loss(loss_ctr2d_res)
        self.loss_kpt2d_ofs = build_loss(loss_kpt2d_ofs)
        self.loss_kpt2d_hmp = build_loss(loss_kpt2d_hmp)
        self.loss_kpt2d_res = build_loss(loss_kpt2d_res)
        self.loss_dim3d = build_loss(loss_dim3d)
        if 'Aware' in loss_dim3d['type']:
    ...
    def _build_head(self, chi, chf, cho):
        head_convs = nn.Sequential(
            nn.Conv2d(chi, chf, kernel_size=3, padding=1),
            self._build_norm(chf), nn.ReLU(inplace=True),
            nn.Conv2d(chf, cho, kernel_size=1)
        )
        return head_convs

    def _build_norm(self, chf):
        if not self.use_attn_norm:
            norm = self.norm(chf, momentum=0.03, eps=0.001)
        else:
            norm = self.norm(chf, self.num_attn_norm, momentum=0.03, eps=0.001)
        return norm

    def _build_dir_head(self, in_channels, feat_channels):
        self.dir_feat = nn.Sequential(
            nn.Conv2d(in_channels, feat_channels, kernel_size=3, padding=1),
            self._build_norm(feat_channels),
            nn.ReLU(inplace=True),
        )
        self.dir_cls = nn.Sequential(
            nn.Conv2d(feat_channels, self.num_alpha_bins, kernel_size=1)
        )
    ...
    def init_weights(self):
        ...
        for head in [
            self.wh_head, self.offset_head, self.center2kpt_offset_head,
            self.depth_head, self.kpt_heatmap_offset_head, self.dim_head,
            self.dir_feat, self.dir_cls, self.dir_reg
        ]:
    ...
    def forward_single(self, feat):
        ...
        center_heatmap_pred = torch.clamp(
            center_heatmap_pred, min=1e-4, max=1 - 1e-4
        )
        ...
        depth_pred[:, 0, :, :
                   ] = 1. / (depth_pred[:, 0, :, :].sigmoid() + self.eps) - 1
    ...
    @force_fp32(
        apply_to=(
            ...
            'center2kpt_offset_preds', 'kpt_heatmap_preds',
            'kpt_heatmap_offset_preds', 'dim_preds', 'alpha_cls_preds',
    ...
    def loss(
        ...
        target_result = self.get_targets(
            gt_bboxes, gt_labels, gt_bboxes_3d, centers2d, depths, gt_kpts_2d,
            gt_kpts_valid_mask, center_heatmap_pred.shape,
            img_metas[0]['pad_shape'], img_metas
        )
        ...
        # 2d offset
        offset_pred = self.extract_input_from_tensor(
            offset_pred, indices, mask_target
        )
        offset_target = self.extract_target_from_tensor(
            offset_target, mask_target
        )
        # 2d size
        ...
        # 3d dim
        dim_pred = self.extract_input_from_tensor(
            dim_pred, indices, mask_target
        )
        dim_target = self.extract_target_from_tensor(dim_target, mask_target)
        # depth
        depth_pred = self.extract_input_from_tensor(
            depth_pred, indices, mask_target
        )
        depth_target = self.extract_target_from_tensor(
            depth_target, mask_target
        )
        # alpha cls
        alpha_cls_pred = self.extract_input_from_tensor(
            alpha_cls_pred, indices, mask_target
        )
        alpha_cls_target = self.extract_target_from_tensor(
            alpha_cls_target, mask_target
        ).type(torch.long)
        alpha_cls_onehot_target = alpha_cls_target.new_zeros([
            len(alpha_cls_target), self.num_alpha_bins
        ]).scatter_(dim=1, index=alpha_cls_target.view(-1, 1), value=1)
        # alpha offset
        alpha_offset_pred = self.extract_input_from_tensor(
            alpha_offset_pred, indices, mask_target
        )
        alpha_offset_pred = torch.sum(
            alpha_offset_pred * alpha_cls_onehot_target, 1, keepdim=True
        )
        alpha_offset_target = self.extract_target_from_tensor(
            alpha_offset_target, mask_target
        )
        # center2kpt offset
        center2kpt_offset_pred = self.extract_input_from_tensor(
            center2kpt_offset_pred, indices, mask_target
        )  # B * (num_kpts * 2)
        center2kpt_offset_target = self.extract_target_from_tensor(
            center2kpt_offset_target, mask_target
        )
        mask_center2kpt_offset = self.extract_target_from_tensor(
            mask_center2kpt_offset, mask_target
        )
        # kpt heatmap offset
        kpt_heatmap_offset_pred = transpose_and_gather_feat(
            kpt_heatmap_offset_pred, indices_kpt
        )
        ...
        mask_kpt_heatmap_offset = self.extract_target_from_tensor(
            mask_kpt_heatmap_offset, mask_target
        )

        # calculate loss
        loss_ctr2d_hmp = self.loss_ctr2d_hmp(
            center_heatmap_pred, center_heatmap_target
        )
        loss_kpt2d_hmp = self.loss_kpt2d_hmp(
            kpt_heatmap_pred, kpt_heatmap_target
        )

        loss_dim2d = self.loss_dim2d(wh_pred, wh_target)
        loss_ctr2d_res = self.loss_ctr2d_res(offset_pred, offset_target)
        if self.dim_aware_in_loss:
            loss_dim3d = self.loss_dim3d(dim_pred, dim_target, dim_pred)
        else:
            loss_dim3d = self.loss_dim3d(dim_pred, dim_target)

        depth_pred, depth_log_variance = depth_pred[:, 0:1], depth_pred[:, 1:2]
        loss_depth = self.loss_depth(
            depth_pred, depth_target, depth_log_variance
        )

        center2kpt_offset_pred *= mask_center2kpt_offset
        loss_kpt2d_ofs = self.loss_kpt2d_ofs(
            center2kpt_offset_pred,
            center2kpt_offset_target,
            avg_factor=(mask_center2kpt_offset.sum() + self.eps)
        )

        kpt_heatmap_offset_pred *= mask_kpt_heatmap_offset
        loss_kpt2d_res = self.loss_kpt2d_res(
            kpt_heatmap_offset_pred,
            kpt_heatmap_offset_target,
            avg_factor=(mask_kpt_heatmap_offset.sum() + self.eps)
        )

        if mask_target.sum() > 0:
            loss_alpha_cls = self.loss_alpha_cls(
                alpha_cls_pred, alpha_cls_onehot_target
            )
        else:
            loss_alpha_cls = 0.0
        loss_alpha_reg = self.loss_alpha_reg(
            alpha_offset_pred, alpha_offset_target
        )

        return dict(
            loss_ctr2d_hmp=loss_ctr2d_hmp,
            loss_dim2d=loss_dim2d,
            loss_ctr2d_res=loss_ctr2d_res,
            loss_dim3d=loss_dim3d,
            loss_kpt2d_ofs=loss_kpt2d_ofs,
            loss_kpt2d_hmp=loss_kpt2d_hmp,
            loss_kpt2d_res=loss_kpt2d_res,
    ...
    def get_targets(
        self, gt_bboxes, gt_labels, gt_bboxes_3d, centers2d, depths, gt_kpts_2d,
        gt_kpts_valid_mask, feat_shape, img_shape, img_metas
    ):
        ...
        center2kpt_offset_target = gt_bboxes[-1].new_zeros([
            bs, self.max_objs, self.num_kpts * 2
        ])
        kpt_heatmap_target = gt_bboxes[-1].new_zeros([
            bs, self.num_kpts, feat_h, feat_w
        ])
        kpt_heatmap_offset_target = gt_bboxes[-1].new_zeros([
            bs, self.max_objs, self.num_kpts * 2
        ])

        # indices
        indices = gt_bboxes[-1].new_zeros([bs, self.max_objs]
                                          ).type(torch.cuda.LongTensor)
        indices_kpt = gt_bboxes[-1].new_zeros([
            bs, self.max_objs, self.num_kpts
        ]).type(torch.cuda.LongTensor)

        # masks
        mask_target = gt_bboxes[-1].new_zeros([bs, self.max_objs])
        mask_center2kpt_offset = gt_bboxes[-1].new_zeros([
            bs, self.max_objs, self.num_kpts * 2
        ])
        mask_kpt_heatmap_offset = gt_bboxes[-1].new_zeros([
            bs, self.max_objs, self.num_kpts * 2
        ])
        ...
        for batch_id in range(bs):
            ...
            gt_kpt_2d = gt_kpt_2d.reshape(-1, self.num_kpts, 2)
            ...
            for j, ct in enumerate(gt_centers):
                ...
                ind = gt_label[j]
                gen_gaussian_target(
                    center_heatmap_target[batch_id, ind], [ctx_int, cty_int],
                    radius
                )
                ...
                alpha_cls_target[batch_id, j], alpha_offset_target[
                    batch_id, j] = self.angle2class(alpha)
                ...
                for k in range(self.num_kpts):
                    ...
                    if vis_level < self.vis_level_regs:
                        continue

                    center2kpt_offset_target[batch_id, j,
                                             k * 2] = kptx - ctx_int
                    center2kpt_offset_target[batch_id, j,
                                             k * 2 + 1] = kpty - cty_int
                    mask_center2kpt_offset[batch_id, j, k * 2:k * 2 + 2] = 1

                    is_kpt_inside_image = (0 <= kptx_int <
                                           feat_w) and (0 <= kpty_int < feat_h)
                    if not is_kpt_inside_image:
                        continue

                    gen_gaussian_target(
                        kpt_heatmap_target[batch_id, k], [kptx_int, kpty_int],
                        radius
                    )
                    ...
                    kpt_heatmap_offset_target[batch_id, j,
                                              k * 2] = kptx - kptx_int
                    kpt_heatmap_offset_target[batch_id, j,
                                              k * 2 + 1] = kpty - kpty_int
    ...
    def angle2class(self, angle):
        ...
        angle_per_class = 2 * np.pi / float(self.num_alpha_bins)
    ...
    def decode_heatmap(
        ...
        *batch_dets, ys, xs = get_topk_from_heatmap(center_heatmap_pred, k=k)
        ...
        center2kpt_offset = transpose_and_gather_feat(
            center2kpt_offset_pred, batch_index
        )
        center2kpt_offset = center2kpt_offset.view(batch, k,
                                                   self.num_kpts * 2)[..., -2:]
        ...
        # 2. recover rotY
        rot_y = self.recover_rotation(
            kpts, alpha, camera_intrinsic
        )  # (b, k, 3)

        # 2.5 recover box3d_center from center2d and depth
        center3d = torch.cat([center2d, depth], dim=-1).squeeze(0)
        center3d = self.pts2Dto3D(center3d,
                                  np.array(camera_intrinsic)).unsqueeze(0)
    ...
    def pts2Dto3D(points, view):
        ...
        homo_points2D = torch.cat([
            unnorm_points2D,
            points2D.new_ones((nbr_points, 1))
        ],
                                  dim=1)
    ...
    def forward_train(
        ...
        **kwargs
        ...
        loss_inputs = outs + (
            gt_bboxes, gt_labels, gt_bboxes_3d, gt_labels_3d, centers2d, depths,
            gt_kpts_2d, gt_kpts_valid_mask, img_metas, attr_labels
        )
...
class MonoConHeadInference(MonoConHead):
    ...
    def init_weights(self):
        ...
        self.heatmap_head[-1].bias.data.fill_(bias_init)  # -2.19
        for head in [
            self.center2kpt_offset_head, self.depth_head, self.dim_head,
            self.dir_feat, self.dir_cls, self.dir_reg
        ]:
            for m in head.modules():
                if isinstance(m, nn.Conv2d):
    ...
    @force_fp32(
        apply_to=(
            'center_heatmap_preds', 'center2kpt_offset_preds', 'dim_preds',
            'alpha_cls_preds', 'alpha_offset_preds', 'depth_preds'
        )
    )
